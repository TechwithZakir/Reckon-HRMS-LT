...
def get_data(filters):
	...
	data = frappe.db.sql(f"""
		SELECT 
			...
			a.employee_name,
			a.attendance_date,
			DATE_FORMAT(MIN(ec.time), '%%H:%%i') as check_in,
			DATE_FORMAT(MAX(ec.time), '%%H:%%i') as check_out,
			a.working_hours,
			a.status
		...
		WHERE a.docstatus = 1
		{conditions}
		GROUP BY a.name, a.employee, a.employee_name, a.attendance_date, a.working_hours, a.status
