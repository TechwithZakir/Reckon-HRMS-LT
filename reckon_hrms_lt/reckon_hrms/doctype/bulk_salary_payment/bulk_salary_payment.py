...
class BulkSalaryPayment(Document):
	...
	def update_status(self):
		"""Update bulk salary status based on child rows."""
		rows = [r for r in self.bulk_salary_employees or [] if r.salary_payment]
		if not rows:
			return
		if all(r.payment_status == "Paid" for r in rows):
			self.db_set("status", "Paid")
		elif any(r.payment_status == "Paid" for r in rows):
			# partial success keeps Submitted so unpaid rows can be retried
	...
	def pay_all(self):
		...
		frappe.enqueue(
			"reckon_hrms_lt.reckon_hrms.doctype.salary_payment.salary_payment.pay_salary_payments",
			names=names,
			queue="long",
			timeout=3600,
		)
