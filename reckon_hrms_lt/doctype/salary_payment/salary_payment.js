...
frappe.ui.form.on("Salary Payment", {
	...
	employee(frm) {
		...
		frappe.call({
			method: "reckon_hrms_lt.doctype.salary_payment.salary_payment.get_default_gross_salary",
	...
	do_pay(frm) {
		...
		d.fields_dict.summary.$wrapper.html(
			...
			${__("Net Salary")}: <b>${format_currency(frm.doc.net_salary, frm.doc.company_currency)}</b></div>`
		);
		d.show();
	},
